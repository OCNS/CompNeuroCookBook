...
The review template can be found at the end of the document.
...
- The software **should** include `Code of Conduct guidelines <https://www.cnsorg.org/code-of-conduct>`__.
...
- The source code for the software **must** be hosted on a publicly accessible resource, and should be freely downloadable.
  ...
  - `GitHub <github.org>`__,
  - `GitLab <gitlab.com>`__
  - `Bitbucket <bitbucket.org>`__
  - `Sourceforge <sourceforge.net>`__
...
- The software **must** have user documentation.
  ...
  - `GitLab pages <https://about.gitlab.com/stages-devops-lifecycle/pages/>`__.
...
- The software **must** have support channels.
  ...
  - `Gitter <gitter.im>`__, (FOSS, free of cost)
  - `Matrix <https://matrix.org/>`__, (FOSS, free of cost)
  - `IRC via Freenode <https://freenode.net/>`__, (FOSS, free of cost)
  - `Discord <https://discord.com/>`__, (not FOSS, free of cost to use with paid tiers)

  While `Slack <slack.com>`__ is a mainstream collaborative chat platform used by small teams nowadays, it is not recommended for public end user troubleshooting:

  - does not scale to larger numbers of users,
  - requires an invitation to join (or setting up an invitation management website),
  - is not FOSS,

  A FOSS alternative is `Mattermost <https://mattermost.com//>`__ which is FOSS, can be self-hosted and also provides paid hosting.

  Forums/mailing list platforms:

  - `Discourse <discourse.org>`__, (can be self hosted, paid hosting available)
  - `GNU Mailman <http://www.list.org/>`__, (FOSS, commonly used, but generally requires hosting)
  - `Google Groups <https://groups.google.com/>`__, (free of cost, but not FOSS)
...
- The software **must** be licensed under a Free/Open Source Software (FOSS) license.
  ...
  - `Free Software Foundation license list <https://www.gnu.org/licenses/license-list.html>`__,
  - `Debian license list <https://www.debian.org/legal/licenses/>`__.
  - `Fedora project license list <https://fedoraproject.org/wiki/Licensing:Main?rd=Licensing#Good_Licenses>`__,
  - `TLDR legal: software license in plain English <https://tldrlegal.com/>`__.
  - `licensecheck: simple licensechecker for source files <https://metacpan.org/pod/distribution/App-Licensecheck/bin/licensecheck>`__.
...
.. literalinclude:: ReviewTemplate.rst
   :language: md
